...
- `chunk_blocks(blocks)` — structure-aware packer. Consumes the
  `blocks: list[{type, content, reading_order}]` that doc-processor returns
  and packs them into chunks along structural boundaries:
...
per-chunk metadata (block_types, section_title) for observability and
filtered retrieval.
