...
def summarize_prompt(combined_text: str, language: str = "ar") -> str:
    ...
    return (
        ...
        "- indicators: up to 30 of the most important/significant quantitative figures in the report "
        "(prioritise headline totals and key breakdowns — skip granular row-level data). "
        "Each item must include: name, value, unit (if any), time_period (if any), context (country/sector/scope if any).\n"
        "- trends: up to 15 of the most significant qualitative directional patterns over time. Each item must include: "
