...
async def trigger_ingest(job_id: str, report_id: str, file_url: str) -> None:
    ...
    payload = {
        ...
        "options": {
            ...
            "extract_formulas":  True,
            "ocr_fallback":      True,
            # Re-enabled 2026-05-08 after switching from local Florence-2
            # to Vertex Gemini Vision. Cost dropped from ~5-10 s of GPU
            # per figure (Florence-2) to ~0.5-1.5 s of network + Vertex
            # inference per figure. Still gated by area threshold in the
            # orchestrator (figure_caption_min_area_ratio = 2% of page),
            # so tiny figures / logos are skipped.
            "figure_captioning": True,
        },
    }
